...
if __name__ == "__main__":
    ...
    if cfg.dataset == 'CULane':
        ...
        splits = ['test.txt']
        #Change the path to input images folder
    ...
    for split, dataset in zip(splits, datasets):
        ...
        for index, data in enumerate(tqdm.tqdm(loader)):
            imgs, names = data
            imgs = imgs.cuda()
            with torch.no_grad():
                out = net(imgs)
                ...
                if np.sum(out_j[:, i] != 0) > 2:
                    for k in range(out_j.shape[0]):
                        if out_j[k, i] > 0:
                            ...
                            cv2.circle(vis,ppp,5,(255,255,0),-1)
            cv2.imwrite(f"OUTPUTS/image_{index}.jpg", vis)
